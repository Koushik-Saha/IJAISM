...
export default function NewBookSubmissionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success("Book manuscript submitted successfully!");
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      <div className="bg-white border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">New Book Manuscript Submission</h1>
            <p className="text-sm text-gray-500 mt-1">Submit a new book manuscript to the submissions pipeline.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent text-sm"
                placeholder="Keywords separated by commas (e.g. engineering, computer science)"
            ...
            >
              {isSubmitting ? "Submitting..." : "Submit Book Manuscript"}
            </button>
          </form>
        </div>
      </div>
